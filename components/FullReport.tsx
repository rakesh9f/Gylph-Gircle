...
const FullReport: React.FC<FullReportProps> = ({ reading, title, subtitle, imageUrl }) => {
  ...
  const handleEmailReport = () => {
      ...
      // Clean up text for email body (remove markdown symbols)
  ...
  // Function to parse text and render it nicely, stripping headers
  const renderFormattedText = (text: string) => {
    return text.split('\n').map((line, i) => {
      if (!line.trim()) return <br key={i} />;
      
      // Clean leading hashes if AI ignored instructions
      ...
      return (
        <p key={i} className={`mb-3 text-justify leading-relaxed ${cleanLine.startsWith('-') || cleanLine.match(/^\d+\./) ? 'pl-4' : ''}`}>
          {parts.map((part, j) => {
            if (part.startsWith('**') && part.endsWith('**')) {
              // Section Titles often come as standalone bold lines, make them stand out
              if (cleanLine === part) {
                  return <strong key={j} className="block text-amber-300 text-lg mt-4 mb-2 print:text-black print:text-xl print:font-bold">{part.slice(2, -2)}</strong>;
              }
              return <strong key={j} className="text-amber-400 font-bold print:text-black">{part.slice(2, -2)}</strong>;
            }
  ...
  return (
    <>
      {/* Screen View */}
      <div className="animate-fade-in-up w-full print:hidden">
        ...
        <div className="text-left bg-black/40 p-6 rounded-lg border border-amber-500/10 mb-6 relative overflow-hidden group">
              ...
              <div className={`text-amber-100/90 font-lora leading-relaxed space-y-4 ${imageUrl ? 'md:flex md:gap-6' : ''}`}>
                 {imageUrl && (
      ...
      <div className="hidden print:block text-black bg-white p-8">
          <div className="text-center mb-8 border-b-2 border-black pb-4">
              <h1 className="text-4xl font-bold font-cinzel text-black mb-2">Glyph Circle</h1>
              <h2 className="text-2xl font-serif text-gray-700">{title}</h2>
              {subtitle && <p className="text-sm font-bold uppercase tracking-widest text-gray-600 mt-2">{subtitle}</p>}
          </div>

          <div className="flex gap-6 mb-6">
             {imageUrl && (
                 <div className="w-1/3">
                     <img src={imageUrl} alt="Chart" className="w-full border border-gray-300 rounded" />
                 </div>
             )}
             <div className={imageUrl ? 'w-2/3' : 'w-full'}>
                 <div className="font-serif text-justify leading-relaxed text-black space-y-4">
                    {renderFormattedText(reading)}
                 </div>
             </div>
          </div>

          <div className="mt-8 border-t-2 border-black pt-4 flex justify-between text-sm text-gray-600">
              <p>Generated by Glyph Circle</p>
