      ...
      <div className="container mx-auto px-4 py-3 flex flex-wrap justify-between items-center gap-4">
        <div className="flex items-center gap-4">
            <Link to="/home" className="cursor-pointer flex-shrink-0 group">
                <h1 className={`${isMobile ? 'text-xl' : 'text-2xl md:text-3xl'} font-bold text-amber-400 tracking-wider font-cinzel group-hover:text-amber-300 transition-colors`}>
                  {t('glyphCircle')}
                </h1>
            </Link>
            
            {/* Hide History Link in Header on Mobile (moved to Bottom Bar) */}
            {!isMobile && (
              <Link 
                  to="/history" 
        ...
        <div className="flex items-center gap-3 md:gap-4 flex-wrap justify-end">
            ...
            {isAdmin && (
                <>
                    ...
                    <button 
                        onClick={() => setIsMasterToolsOpen(true)}
                        className="md:hidden flex items-center justify-center w-9 h-9 rounded-full bg-gray-800 text-amber-400 border border-amber-500/30 hover:bg-amber-900"
                    >
                        ⚙️
                    </button>
            ...
            {/* Language Switcher - Hidden on mobile header, usually absolute positioned in App.tsx or handled separately */}
            {!isMobile && (
            ...
            <button
              onClick={onLogout}
              className={`
                bg-maroon-700/80 hover:bg-maroon-600 border border-amber-500/50 text-amber-100 font-bold rounded-lg transition duration-300 ease-in-out transform hover:scale-105 text-sm whitespace-nowrap
